...
public class LoginViewModel
{
    ...
    public void TryToEnter(string login, string password)
    {
        ...
        if (_usersData.UsersList.Any(user => user.Authorization == login && user.Password == password))
        {
            var user = _usersData.UsersList.Single(user => user.Authorization == login && user.Password == password);
